...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->BitNet
...
class BitNetLinear(nn.Module):
    """BitNet linear layer with quantization"""
...
class BitNetAttention(nn.Module):
    ...
    def __init__(self, config: BitNetConfig, layer_idx: Optional[int] = None):
        ...
        self.k_proj = BitNetLinear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=False)
        self.v_proj = BitNetLinear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=False)
        self.o_proj = BitNetLinear(self.hidden_size, self.hidden_size, bias=False)
        self._init_rope()
    ...
    def _update_causal_mask(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if attention_mask.max() != 0:
                raise ValueError("Custom 4D attention mask should be passed in inverted form with max value of 0")
    ...
    def prepare_inputs_for_generation(
        ...
        if self.generation_config.cache_implementation == "static":
            ...
            if cache_position is None:
                ...
                cache_position = torch.arange(
                    past_length, past_length + input_ids.shape[1], device=input_ids.device, dtype=torch.long
                )
